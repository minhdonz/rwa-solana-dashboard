    ...
    <div className="space-y-8">
      <header className="max-w-3xl border-b border-line pb-6">
        ...
        <p className="text-slate-600 leading-relaxed mt-3">
          A &quot;tokenized AAPL&quot; is not one instrument. The same share is wrapped differently by each
          issuer, and that decides whether a holder can vote, collect dividends, redeem for a real share,
          or use the token in DeFi. The matrix compares the major Solana issuers on the dimensions that
          matter to a holder.
        </p>
      ...
      <section className="border-l-2 border-brand bg-surface px-5 py-4 max-w-3xl">
        <h3 className="font-semibold text-navy text-sm">Buying: primary vs. secondary</h3>
        <div className="text-sm text-slate-600 mt-2 leading-relaxed space-y-2">
          <p>
            <span className="font-medium text-slate-700">Primary</span> (mint/redeem with the issuer)
            settles on US-equity rails, so it runs <span className="tnum">24/5</span> and stops on weekends.{" "}
            <span className="font-medium text-slate-700">Secondary</span> (buying existing tokens on a DEX
            or CEX) runs <span className="tnum">24/7</span>.
          </p>
          <p>
            xStocks and Backpack have deep secondary markets, so you can buy any time (price may drift from
            the underlying while primary is closed). Ondo has almost no secondary liquidity, so its{" "}
            <span className="tnum">24/5</span> intent mechanism is the only way in: no weekend buying.
          </p>
        </div>
      </section>
